...
void SortedMergeRecur(Node*& head_x, Node*& head_y, Node*& head_z)
{
   // Stop recursion
   if (head_y == 0 && head_x == 0) return;

   Node* cursor = 0;
   // Case 1 both are nodes
   if (head_x != 0 && head_y != 0)
   {
      if (head_x->data > head_y->data)
      {
         cursor = head_y;
         head_y = head_y->link;
      }
   ...
   cursor->link = head_z;
   head_z = cursor;
   SortedMergeRecur(head_x, head_y, head_z->link);
}
